...
trait ErrorsTrait
{
    ...
    protected function handleNetgsmErrors(string $work_type, string $response): void
    {
        ...
        if (property_exists(self::class, $property)) {
            ...
            $error_code = is_array($response) ? $response["code"] : $response;

            if (substr($error_code, 0, 1) !== "0") {

                NetgsmLogger::$response_code = $error_code;

                if (array_key_exists($error_code, self::${$property})) {

                    NetgsmLogger::$response_message = self::${$property}[$error_code];

                } else {

                    if (isset($response["error"])) {

                        NetgsmLogger::$response_message = $response["error"];

                    } else {

                        NetgsmLogger::$response_message = "An unknown error occurred.";
                ...
                throw new NetgsmException(NetgsmLogger::$response_message, 422);
        ...
        if (strpos($response, '<?xml') !== FALSE) {
            ...
            foreach ($xml->main->children() as $xml_node_name => $xml_node_value) {
                ...
                if ($xml_node_name == 'code') {
                    ...
                    $code_with_error["code"] = (string)$xml_node_value;

                }

                if ($xml_node_name == 'error') {

                    $code_with_error["error"] = (string)$xml_node_value;

                }
            }

            return $code_with_error;
        }
